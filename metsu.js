...
// ===== 滅ボタンの有効/無効を判定 =====
// top50 = topIndex が 49（0始まり）
// かつ スロットが止まっている（currentStep が "top" でない）
window.updateMetsuBtn = function () {
  const btn = document.getElementById('metsu-btn');
  if (!btn) return;

  const isTop50 = (window.topIndex === 49);
  const isStopped = (window.currentStep !== "top");
